...
WITH VU_alumni AS
	...
	JOIN schools AS s 
    	USING (schoolid)
	WHERE s.schoolname ILIKE '%Vanderbilt%'    -----Players who are Vanderbilt University alumni 
	)
SELECT SUM(salary) AS total_salary   ---TO_CHAR converts to text string using 
	   ,full_name                                              --$999,999,999 as a "guide"
FROM salaries
	    JOIN VU_alumni
		USING (playerid)
...
FROM salaries
		...
		USING (playerid)
GROUP BY full_name, playerid
ORDER BY total_salary DESC NULLS LAST ---Joining VU_alumni CTE with salary totals
...
SELECT namefirst||' '||namelast AS full_name
	  ,SUM(sb)+SUM(cs)AS steal_attempts
	  ,ROUND(SUM(sb::numeric)/(SUM(sb::numeric)+SUM(cs::numeric))*100,2) AS steal_percentage
FROM batting
...
--	   games). Only consider parks where there were at least 10 games played. Report the park name, team name, and
...
WITH yearly_salary AS
	(SELECT SUM(salary)::INTEGER AS salary_per_year
		   ,yearid
		   ,teamid
	FROM salaries
...
SELECT 
    t.yearid,
    CORR(t.w, ys.salary_per_year) AS correlation
FROM teams AS t
JOIN yearly_salary AS ys 
  ON t.teamid = ys.teamid
 AND t.yearid = ys.yearid
WHERE t.yearid >= 2000
GROUP BY t.yearid
ORDER BY t.yearid DESC;---A slightly positive relationship, not a strong one
...
--     determine just how rare left-handed pitchers are compared with right-handed pitchers. Are left-handed pitchers more
--     likely to win the Cy Young Award? Are they more likely to make it into the hall of fame?
